...
{	
	...
	{
		this.table.workingScope().interpret(new Quantifier(ctx.string().getText()), ctx.name().getText(), VariableType.STRING);
	}
	...
	public void exitVariable_declaration_list(MicroParser.Variable_declaration_listContext ctx)
	{
		...
		if(ctx.variable_type().getText().equals("INT"))
		{
			...
			{
				this.table.workingScope().interpret(new Quantifier(), var, VariableType.INT);
			}
		}
		else
		{
			for(String var : array)
			{
				this.table.workingScope().interpret(new Quantifier(), var, VariableType.FLOAT);
			}
		}		
	}
	...
	public void enterParameter_declaration(MicroParser.Parameter_declarationContext ctx)
	{
		if(ctx.variable_type().getText().equals("INT"))
		{
			this.table.workingScope().interpret(new Quantifier(), ctx.name().getText(), VariableType.INT);
		}
		else
		{
			this.table.workingScope().interpret(new Quantifier(), ctx.name().getText(), VariableType.FLOAT);
		}
	}
