 ...
 public class TinyDancer
 {
   ...
   private ArrayList<String> holdingCells = new ArrayList<String>();
   protected Map<String, Map<String, String>> TonyDanza = new LinkedHashMap<String, Map<String, String>>();
   protected Map<String, Map<String, NodeStructure>> tableMap = new LinkedHashMap<String, Map<String, NodeStructure>>();
   protected Map<String, Integer> linkCount = new LinkedHashMap<String, Integer>();
   protected Map<String, ArrayList<String>> tempMap = new LinkedHashMap<String, ArrayList<String>>();
   protected Map<String, String> registerAllocate = new LinkedHashMap<String, String>();
   private Stack<CFGNode> CFG = new Stack<CFGNode>();
   private ArrayList<String> usedTemp = new ArrayList<String>();
   private int loopPosition = 0;
   
   public TinyDancer(ArrayList<String> input, SymbolTable information_table, Map<String, Map<String, NodeStructure>> inputMap, Map<String, ArrayList<String>> tempMap, Stack<CFGNode> CFG, int registerNumber, int temporary_Number)
   {
     ...
     {
       Map<String, String> newTR = new LinkedHashMap<String, String>();
       ...
       if (elements[0].equalsIgnoreCase("STOREI"))
       {
         if ((elements[1].contains("$")) && (elements[2].contains("$")))
         {
           if ((elements[2].contains("$R")) || (elements[2].contains("$P"))) {
             this.result.add("move " + createTemp(elements[1]) + " " + (String)((Map<String, String>)this.TonyDanza.get(this.labelIndicator)).get(elements[2]) + "\n");
           } else {
   ...
   private void spillRegister()
   {
     for (int k = 0; k < this.registerAllocate.size(); k++) {
       ...
       {
         this.result.add("move r" + Integer.toString(k) + " " + (String)((Map<String, String>)this.TonyDanza.get(this.labelIndicator)).get(this.registerAllocate.get(new StringBuilder("r").append(Integer.toString(k)).toString())) + "\n");
       ...
       if (scope.type.equalsIgnoreCase("GLOBAL")) {
         for (String key : scope.symbolMap.keySet()) {
           if (((BuildNode)scope.symbolMap.get(key)).type == VariableType.INT) {
             ...
             this.result.add("str " + key + " " + ((BuildNode)scope.symbolMap.get(key)).descriptor.content + "\n");
           } else {
   ...
   public String createTemp(String temp)
   {
     if (((Map<?, ?>)this.TonyDanza.get(this.labelIndicator)).containsKey(temp))
     {
       ...
       else
       {
         if (registerFull())
         {
           ...
           {
             this.result.add("move " + tempR + " " + (String)((Map<String, String>)this.TonyDanza.get(this.labelIndicator)).get(this.registerAllocate.get(tempR)) + "\n");
             this.usedTemp.add((String)this.registerAllocate.get(tempR));
             this.registerAllocate.put(tempR, temp);
             if (this.usedTemp.contains(temp))
             {
               this.result.add("move " + (String)((Map<String, String>)this.TonyDanza.get(this.labelIndicator)).get(temp) + " " + tempR + "\n");
               this.usedTemp.remove(temp);
             }
             else
             {
         ...
         for (int i = 0; i < this.registerAllocate.size(); i++) {
           if (this.registerAllocate.get("r" + Integer.toString(i)) == null)
           {
             ...
             if (temp.contains("P")) {
               this.result.add("move " + (String)((Map<String, String>)this.TonyDanza.get(this.labelIndicator)).get(temp) + " " + "r" + Integer.toString(i) + "\n");
             }
             if (this.usedTemp.contains(temp))
             {
               this.result.add("move " + (String)((Map<String, String>)this.TonyDanza.get(this.labelIndicator)).get(temp) + " " + "r" + Integer.toString(i) + "\n");
     ...
     else
     {
       if (((Map<?, ?>)this.TonyDanza.get("GLOBAL")).containsKey(temp)) {
         return (String)((Map<String, String>)this.TonyDanza.get("GLOBAL")).get(temp);
       }
